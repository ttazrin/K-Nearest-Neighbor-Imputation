...
def KNN(datasetKNN,distanceList):
    #minDistIdx = max(set(distanceList[0:10][0])) #, key =  Edistance[0:5][1].count) //most occurred number in smallest 5 distance
    #indexDist = distanceList[0].index(minDistIdx)
    #minDistIdx = distanceList[indexDist][1]
    #print(minDistIdx)
    k=10
    if featurePointer==0:
        modeList= []
...
def numericalAccuracy(ds):
    ...
    for i in naIndex:
        ...
        difference = difference + abs(abs(df.iloc[i,featurePointer])-abs(ds.iloc[i,featurePointer]))
...
while(featurePointer!=3):
    ...
    std_scale = preprocessing.StandardScaler().fit(data[['Temperature (C)', 'Humidity']])
    data[['Temperature (C)', 'Humidity']] = std_scale.transform(data[['Temperature (C)', 'Humidity']])
    
    #Scale numerical features using MAX MIN SCALING
    minmax_scale = preprocessing.MinMaxScaler().fit(data[['Temperature (C)', 'Humidity']])
    df_minmax = minmax_scale.transform(df[['Temperature (C)', 'Humidity']])
...
import csv
with open('resultsFromCodeAfterScaling.csv', 'w') as csvFile:
    writer = csv.writer(csvFile)
    writer.writerows(accuracyList)

print("Results are saved in resultsFromCodeAfterScaling.csv file")